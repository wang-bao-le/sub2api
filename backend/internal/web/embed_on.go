...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	// Check the internal cache first. The cached content keeps the nonce
	// placeholder and is rendered with the current request nonce below.
	cached := s.cache.Get()
	if cached != nil {
		// Replace nonce placeholder with actual nonce before serving
		content := replaceNoncePlaceholder(cached.Content, nonce)

		// Do not use HTTP ETag/304 caching here. The HTML contains an inline
		// config script whose CSP nonce changes on every request; returning 304
		// can combine a cached body with a new CSP nonce and block the script.
