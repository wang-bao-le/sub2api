...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	...
	t.Run("does_not_set_http_etag_for_nonce_bearing_html", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
	...
	t.Run("returns_full_html_when_client_sends_if_none_match", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		// Use a real router to verify that browser revalidation still receives
		// a complete HTML document with the current nonce.
		router := gin.New()
		...
		// Second request with a stale client validator
		w2 := httptest.NewRecorder()
