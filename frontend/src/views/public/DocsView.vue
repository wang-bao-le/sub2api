<template>
  <div class="min-h-screen bg-gray-50 text-gray-950 dark:bg-dark-950 dark:text-white lg:flex lg:h-screen lg:flex-col lg:overflow-hidden">
    ...
    <main
      ...
      :class="tocItems.length ? 'lg:grid-cols-[calc(220px+1.5cm)_minmax(0,1fr)] xl:grid-cols-[calc(220px+1.5cm)_minmax(0,1fr)_220px]' : 'lg:grid-cols-[calc(220px+1.5cm)_minmax(0,1fr)]'"
    >
      <aside class="hidden border-r border-gray-200 pr-6 dark:border-dark-700 lg:block lg:min-h-0 lg:overflow-y-auto">
        <nav class="space-y-7" aria-label="文档导航">
          <section v-for="group in groups" :key="group.title">
            ...
            <div v-if="group.footerItems?.length" class="space-y-1">
              <RouterLink
                v-for="item in group.footerItems"
                :key="item.slug"
                :to="{ name: 'DocsPage', params: { slug: item.slug } }"
                class="block rounded-xl px-4 py-2 text-sm transition-colors"
                :class="item.slug === currentSlug ? 'bg-gray-900 font-semibold text-white shadow-sm dark:bg-white dark:text-gray-900' : 'text-gray-600 hover:bg-gray-100 dark:text-dark-300 dark:hover:bg-dark-800'"
      ...
      <section class="min-w-0 lg:min-h-0 lg:overflow-y-auto lg:pr-4">
        ...
        <article v-else-if="markdown" class="docs-article card w-full px-6 py-8 sm:px-10 lg:px-12">
...
interface DocGroup { title: string; footerTitle?: string; footerItems?: DocItem[]; items: DocItem[] }
